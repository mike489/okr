...
const Planning = () => {
  ...
  const [weightSum, setWeightSum] = useState(false);
  ...
  const handlePerspectiveCollapsing = (index, kpi) => {
    if (selectedPerspective === index) {
      ...
      handleGettingObjectives(kpi?.perspective_type_id);
      setObjectiveSelected('');
    }
  };
  ...
  const handleSettingUpMeasuringUnitFilter = (measuring_unit) => {
    measuringUnit.length < 2 &&
      measuring_unit.forEach((m_unit) =>
        measuringUnit.push({ label: m_unit.name, value: m_unit.id }),
      );
  };
    ...
    fetch(Api, {
      ...
      .then((response) => {
        if (response.success && response.data?.plans) {
          ...
          setWeightSum(response?.data?.weight_sum);
    ...
    >
        ...
        >
            ...
            {weightSum !== undefined && (
              <DrogaCard
                ...
                sx={{
                  ...
                  fontSize: '16px',
                  bgcolor:
                    weightSum < 100 || weightSum > 100 ? '#ffe5e5' : 'inherit',
                }}
              >
          ...
          {Object.keys(groupedPlans).map((perspectiveType, index) => (
            <Grid item xs={12} key={perspectiveType}>
              <DrogaCard
                onPress={() =>
                  handlePerspectiveCollapsing(
                    index,
                    groupedPlans[perspectiveType][0],
                  )
                }
                sx={{
                  display: 'flex',
              ...
              <Collapse in={selectedPerspective === index}>
                ...
                >
                  <Chip
                    ...
                    sx={{
                      ...
                      ':hover': {
                        bgcolor:
                          objectiveSelected === '' ? 'primary.main' : null,
                        color:
                          objectiveSelected === ''
                            ? 'common.white'
                            : 'common.black',
                        ...
                        sx={{
                          ...
                          color:
                            objectiveSelected === objective.id
                              ...
                              : 'common.black',
                          border:
                            objectiveSelected === objective.id ? 'none' : null,
                          ':hover': {
                            bgcolor:
                              objectiveSelected === objective.id
                                ? 'primary.main'
                ...
                <Grid container spacing={2} mt={0.6}>
                  {FilteredPlan(groupedPlans[perspectiveType]).length === 0 ? (
                    ...
                    FilteredPlan(groupedPlans[perspectiveType]).map(
                      (plan, index) => (
                        <Grid
                          item
                          xs={12}
      ...
      {deletePlan && (
        <DeletePrompt
          ...
          open={deletePlan}
          title="Deleting Plan"
          description={`Are you sure you want to delete ` + selectedPlan?.kpi}
      ...
      >
        {loadingConversations ? (
          ...
          <ErrorPrompt
            title="Server Error"
            message="Unable to retrieve the conversation"
            size={160}
          />
          ...
          <Fallbacks
            ...
            title={``}
            description={`There is no conversation yet`}
            sx={{ paddingTop: 6 }}
            size={80}
          />
        ) : (
        ...
        {!loadingConversations &&
          conversationPagination.total > conversationPagination.per_page && (
            <TablePagination
              component="div"
              rowsPerPageOptions={[]}
